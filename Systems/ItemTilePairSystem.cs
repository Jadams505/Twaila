...
namespace Twaila.Systems
{
    ...
    public class ItemTilePairSystem : ModSystem
    {
        ...
        public static int GetPickId(int pickPower, int startIndex, out int foundIndex)
        {
            ...
            {
                PickPowerPair pick = _pickaxes[i];
        ...
        private static void Populate()
        {
            for (int i = 0; i < ItemID.Count; ++i) // vanilla items
            {
                ...
                if (item.pick > 0)
                {
                    _pickaxes.Add(new PickPowerPair(item.pick, i));
                }
        ...
        */
        ...
        private class PickPowerSorter : IComparer<PickPowerPair>
        {
            public int Compare(PickPowerPair x, PickPowerPair y)
            {
                if (x.power < y.power)
                {
                    return -1;
